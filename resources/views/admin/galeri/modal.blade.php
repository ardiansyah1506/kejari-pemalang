<div id="modal" class="fixed inset-0 flex items-center justify-center hidden backdrop-blur-sm bg-black/30 transition-all duration-300 z-50">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md px-6 transform transition-all">
        <div class="py-4 border-b border-gray-100 flex justify-between items-center">
            <h2 id="modal-title" class="text-xl font-bold text-gray-800">Tambah Galeri</h2>
        ...
        <form id="form-galeri" method="POST" enctype="multipart/form-data" class="py-4">
            @csrf
            <input type="hidden" name="id" id="id">
            <div class="mb-4">
                <label class="font-semibold text-sm text-gray-700 block mb-1.5">Judul</label>
                <input type="text" name="judul" class="border border-gray-300 rounded-xl px-3 py-2.5 text-sm w-full focus:ring-2 focus:ring-[#228d81]/50 focus:border-[#228d81] transition-all outline-none" required />
            </div>
            ...
            <small id="link" class="py-1 flex w-full justify-end text-sm text-gray-500"></small>
